...
import { Sprout, MapPin, ShieldCheck, ArrowRight, User } from 'lucide-react';

export default function FarmersDirectoryPage() {
  const [farmers, setFarmers] = useState<UserProfile[]>([]);

  useEffect(() => {
    const list = SEED_PROFILES.filter((p) => p.role === 'farmer' || p.role === 'fpo');
    setFarmers(list);
  }, []);
          ...
          >
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-base text-white">{farmer.full_name}</h3>
              ...
              <div className="p-3 bg-[#0a0f0d] border border-[#1e2d26] rounded-2xl text-xs space-y-1">
                ...
                <div className="flex justify-between text-gray-400">
                  ...
                  <span className="text-white font-medium">Paddy, Turmeric</span>
                </div>
              </div>
            </div>
