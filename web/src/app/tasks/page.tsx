...
import { CheckSquare, Plus, Calendar, Clock, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
export default function TasksPage() {
  ...
  useEffect(() => {
    dataService.getTasks(user?.id).then(setTasks);
  }, [user]);
  ...
  const handleToggleTask = async (id: string) => {
    await dataService.toggleTaskStatus(id);
    setTasks((prev) =>
