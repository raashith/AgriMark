    ...
    <div className="space-y-6">
          ...
          {filteredListings.map((item) => (
            <div key={item.id} className="bg-[#121a16] border border-[#1e2d26] p-5 rounded-2xl shadow-md space-y-4 hover:border-emerald-800/60 transition flex flex-col justify-between">
              <div className="space-y-2">
                <div className="flex justify-between items-start"><div><h3 className="font-bold text-lg text-emerald-300">{item.crop_name}</h3><p className="text-xs text-gray-400 flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-emerald-400" /> {item.location}</p></div><span className="text-xs px-2 py-1 bg-emerald-950 text-emerald-300 border border-emerald-800/40 rounded font-mono font-bold">{item.quality_grade}</span></div>
                <div className="p-3 bg-[#0a0f0d] border border-[#1e2d26] rounded-xl text-xs space-y-1.5"><div className="flex justify-between items-center text-gray-300 font-semibold"><span>{t('askingPrice')}:</span><span className="text-base font-extrabold text-emerald-400">₹{item.price_per_kg} / KG</span></div><div className="pt-1 border-t border-[#1e2d26] flex justify-between text-[11px] text-gray-400"><span>{t('marketReference')}:</span><span className="font-mono text-amber-300">Live reference when available</span></div><div className="flex justify-between text-[11px] text-gray-400"><span>{t('aiIntelligence')}:</span><span className="font-mono text-purple-300">Decision support only</span></div></div>
                <div className="text-xs text-gray-400 space-y-1 pt-1 font-mono"><p>Available Inventory: <strong>{item.quantity_available_kg} KG</strong></p><p>Min Order Qty: <strong>{item.min_order_quantity_kg} KG</strong></p><p className="text-gray-500">Seller: {item.seller_name || 'Verified Farmer'}</p></div>
              </div>
              <button onClick={() => { setActiveListing(item); setOrderQty(item.min_order_quantity_kg.toString()); setOrderMessage(''); }} className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm rounded-xl shadow transition">{t('placeOrder')}</button>
            </div>
          ))}
        </div>
      )}
      ...
      {activeListing && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4"><div className="bg-[#121a16] border border-[#1e2d26] p-6 rounded-2xl max-w-md w-full shadow-2xl space-y-4"><div className="flex justify-between items-center border-b border-[#1e2d26] pb-3"><h3 className="font-bold text-lg text-emerald-300">Place Wholesale Order</h3><button onClick={() => setActiveListing(null)} className="text-gray-400 hover:text-white">✕</button></div>
          ...
          <div className="space-y-2 text-xs text-gray-300 font-mono bg-[#0a0f0d] p-3 rounded-xl border border-[#1e2d26]"><p><strong>Produce:</strong> {activeListing.crop_name}</p><p><strong>Asking Price:</strong> ₹{activeListing.price_per_kg} / KG</p><p><strong>Available Qty:</strong> {activeListing.quantity_available_kg} KG</p><p><strong>Min Order Qty:</strong> {activeListing.min_order_quantity_kg} KG</p></div>
          <form onSubmit={handlePlaceOrder} className="space-y-4"><div><label className="block text-xs font-semibold uppercase text-gray-400 mb-1">Order Quantity (KG)</label><input type="number" required min={activeListing.min_order_quantity_kg} max={activeListing.quantity_available_kg} value={orderQty} onChange={(e) => setOrderQty(e.target.value)} className="w-full px-4 py-2.5 bg-[#0a0f0d] border border-[#1e2d26] rounded-xl text-white focus:border-emerald-500 focus:outline-none" /></div><div className="p-3 bg-emerald-950/40 border border-emerald-800/40 rounded-xl flex justify-between items-center text-sm font-bold text-emerald-300"><span>Total Amount:</span><span>₹{((parseFloat(orderQty) || 0) * activeListing.price_per_kg).toFixed(2)}</span></div><div className="flex gap-3"><button type="button" onClick={() => setActiveListing(null)} className="flex-1 py-2.5 bg-[#0a0f0d] border border-[#1e2d26] text-gray-300 font-semibold rounded-xl">Cancel</button><button type="submit" disabled={orderSubmitting} className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow transition">{orderSubmitting ? t('loading') : 'Confirm Order'}</button></div></form>
        </div></div>
      )}
    </div>
  );
