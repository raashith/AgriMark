...
import { Sprout, Plus, Calendar, Layers, CheckCircle2 } from 'lucide-react';
...
export default function FarmerCropsPage() {
  ...
  useEffect(() => {
    dataService.getCultivations().then(setCultivations);
    dataService.getCropCatalog().then(setCatalog);
  }, []);
            ...
            <div key={c.id} className="bg-[#121a16] border border-[#1e2d26] rounded-3xl p-6 space-y-4 shadow-lg">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <div className="p-3 bg-[#0a0f0d] border border-[#1e2d26] rounded-xl">
                  ...
                  <p className="font-bold text-white text-xs mt-0.5">{c.season}</p>
                </div>
                <div className="p-3 bg-[#0a0f0d] border border-[#1e2d26] rounded-xl">
                ...
                <div className="p-3 bg-[#0a0f0d] border border-[#1e2d26] rounded-xl">
                  ...
                  <p className="font-bold text-emerald-400 text-xs mt-0.5">{( (c.expected_yield_kg || 5000) / 1000 ).toFixed(1)} Tons</p>
                </div>
              </div>
